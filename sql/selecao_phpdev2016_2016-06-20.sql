...
# Tempo de Geração: 2016-06-20 11:44:07 +0000
# ************************************************************
...
# Dump da tabela vw_animal_vacina
# ------------------------------------------------------------

DROP VIEW IF EXISTS `vw_animal_vacina`;

CREATE TABLE `vw_animal_vacina` (
   `anv_int_codigo` INT(11) UNSIGNED NOT NULL DEFAULT '0',
   `ani_int_codigo` INT(11) UNSIGNED NOT NULL,
   `vac_int_codigo` INT(11) UNSIGNED NOT NULL,
   `anv_dat_programacao` VARCHAR(24) NULL DEFAULT NULL,
   `anv_dti_aplicacao` VARCHAR(24) NULL DEFAULT NULL,
   `anv_dti_inclusao` VARCHAR(24) NULL DEFAULT NULL,
   `usu_int_codigo` INT(11) UNSIGNED NULL DEFAULT NULL,
   `ani_var_nome` VARCHAR(50) NULL DEFAULT NULL,
   `vac_var_nome` VARCHAR(50) NULL DEFAULT NULL
) ENGINE=MyISAM;
...
# Replace placeholder table for vw_animal_vacina with correct view syntax
# ------------------------------------------------------------

DROP TABLE `vw_animal_vacina`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_animal_vacina`
AS SELECT
   `animal_vacina`.`anv_int_codigo` AS `anv_int_codigo`,
   `animal_vacina`.`ani_int_codigo` AS `ani_int_codigo`,
   `animal_vacina`.`vac_int_codigo` AS `vac_int_codigo`,date_format(`animal_vacina`.`anv_dat_programacao`,'%d/%m/%Y %H:%i:%s') AS `anv_dat_programacao`,date_format(`animal_vacina`.`anv_dti_aplicacao`,'%d/%m/%Y %H:%i:%s') AS `anv_dti_aplicacao`,date_format(`animal_vacina`.`anv_dti_inclusao`,'%d/%m/%Y %H:%i:%s') AS `anv_dti_inclusao`,
   `animal_vacina`.`usu_int_codigo` AS `usu_int_codigo`,
   `animal`.`ani_var_nome` AS `ani_var_nome`,
   `vacina`.`vac_var_nome` AS `vac_var_nome`
FROM ((`animal_vacina` left join `animal` on((`animal`.`ani_int_codigo` = `animal_vacina`.`ani_int_codigo`))) left join `vacina` on((`vacina`.`vac_int_codigo` = `animal_vacina`.`vac_int_codigo`)));
